...
|----------|-------------|
| `VANE_MTLS_CA_CERT` | PEM-encoded CA certificate used to verify client certificates. Accepts either the raw PEM string or a file path. |
| `VANE_TLS_CERT` | PEM-encoded server certificate. Accepts PEM string or file path. |
| `VANE_TLS_KEY` | PEM-encoded server private key. Accepts PEM string or file path. |

If `VANE_MTLS_CA_CERT` is set but `VANE_TLS_CERT` or `VANE_TLS_KEY` is missing, the server logs a fatal error and exits.

```bash
VANE_MTLS_CA_CERT=/etc/ssl/vane-ca.pem \
VANE_TLS_CERT=/etc/ssl/vane-server.pem \
VANE_TLS_KEY=/etc/ssl/vane-server-key.pem \
npm start
```
...
```bash
VANE_MTLS_CA_CERT="$(cat /etc/ssl/vane-ca.pem)" \
VANE_TLS_CERT="$(cat /etc/ssl/server.pem)" \
VANE_TLS_KEY="$(cat /etc/ssl/server-key.pem)" \
npm start
```
