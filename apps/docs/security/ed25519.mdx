...
Private keys are stored in PKCS8 PEM format. Public keys are stored in SPKI PEM format. When `COUNSEL_MASTER_KEY` is set, private keys are encrypted with AES-256-GCM before storage. See [Envelope Encryption](/security/envelope-encryption).
