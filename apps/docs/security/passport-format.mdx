...
`PASSPORT_REVOKED` is only returned by server-side verification (`POST /v1/passport/verify`). Offline verification via `@vane/mcp-middleware` does not check revocation — use `GET /v1/ocsp/:jti` for that.
...
interface AttestationReceipt {
  ...
  verifier: string;          // "vane-server/0.1.0" or "@vane/mcp-middleware@0.1.0"
}
```
