...
description: "How COUNSEL_MASTER_KEY protects private keys at rest with AES-256-GCM."
---
...
This produces a 256-bit key from an arbitrary-length passphrase. SHA-256 is used here purely for key stretching from string to bytes — it is not a key derivation function (KDF). For production deployments, use a 64-character random hex string as `COUNSEL_MASTER_KEY` so the entropy is already adequate.
