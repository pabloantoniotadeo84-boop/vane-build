---
title: "@vane.build/mcp-middleware"
...
const vane = createVaneMiddleware({
  vanePublicKey: process.env.VANE_CA_PUBLIC_KEY!, // Ed25519 SPKI PEM
...
|-------|------|---------|-------------|
| `vanePublicKey` | `string` | required | Ed25519 SPKI PEM of the Vane CA. Obtain from `GET /v1/ca/public-key?companyId=<id>`. Pin this value in your deployment. |
...
```typescript
const result = vane.verify(token, {
...
if (result.valid) {
  console.log(result.claims.vane.agentId);
