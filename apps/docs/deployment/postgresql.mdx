...
```bash
DATABASE_URL=postgresql://vane_user:secret@localhost:5432/vane
```
...
CREATE TABLE keys (
  ...
  private_key TEXT NOT NULL  -- AES-256-GCM encrypted if VANE_MASTER_KEY is set
);
```
...
```sql
-- Create the database
CREATE DATABASE vane;

-- Create the user
CREATE USER vane_user WITH PASSWORD 'a-very-strong-password';

-- Grant only what's needed (no superuser, no createdb)
GRANT CONNECT ON DATABASE vane TO vane_user;
\c vane
GRANT USAGE ON SCHEMA public TO vane_user;
GRANT CREATE ON SCHEMA public TO vane_user;
GRANT SELECT, INSERT, DELETE ON ALL TABLES IN SCHEMA public TO vane_user;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO vane_user;
```
...
```bash
DATABASE_URL="postgresql://vane_user:secret@db.example.com:5432/vane?sslmode=require"
```
