...
function buildPreimage(index, timestamp, payload, previousHash) {
...
export class AttestationChain {
    ...
    hydrate(records) {
        if (this.records.length > 0)
            throw new Error('hydrate() must be called on an empty chain');
    ...
    append(payload, privateKeyPem) {
        ...
        const record = { index, timestamp, payload, previousHash, hash, signature };
    ...
    verify(publicKeyPem) {
        for (let i = 0; i < this.records.length; i++) {
            ...
            const expectedHash = sha256hex(buildPreimage(rec.index, rec.timestamp, rec.payload, rec.previousHash));
